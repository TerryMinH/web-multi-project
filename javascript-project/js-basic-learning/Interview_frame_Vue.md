 ...
 * @LastEditTime: 2025-03-23 08:32:20
...
- Vue2 中 watch 与 computed 区别
  ...
  1.  computed
      ...
      1.2 computed 具有缓存机制，只有当依赖的数据发生变化时，才会重新计算。这意味着如果多次访问同一个计算属性，只要依赖的数据没有变化，就会直接返回缓存的结果，避免了重复计算，提高了性能。
      1.3 computed 只能监听数据的浅变化，即当依赖的数据本身发生变化时，计算属性会重新计算
  2.  watch
