 ...
 * @LastEditTime: 2025-04-18 15:32:41
...
- Vue 组件通信有几种方式
  ...
  1.  子组件向父组件传递数据:
      ...
      1.5 状态管理 Vuex/pinia；适用于复杂的状态管理。
      1.6 Vue.observable() 是 Vue 2 中用于创建响应式对象的方法，在 Vue 3 中被 reactive() 替代。它可以将一个普通对象转换为响应式对象，当对象属性发生变化时，依赖这些属性的组件会自动更新。适用场景:适用于一些简单的状态管理和跨组件通信场景。
      1.7 Vue 组件实例属性,访问组件信息：

          - $parent：适用于子组件需要访问父组件数据和方法的场景，但会增加组件间的耦合度。
          - $children：是一个数组，用于父组件需要批量访问所有直接子组件的场景，但缺乏精准性和顺序保障。
          - $refs：适合父组件精准访问特定子组件实例或 DOM 元素，灵活性高。
