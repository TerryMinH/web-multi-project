 ...
 * @LastEditTime: 2025-04-26 09:44:11
...
- JS 常考面试题
  ...
  3. for in 与 for of 区别:
     3.1 for...in：迭代的是对象的键的列表；适用于遍历对象的可枚举属性，包括对象自身和原型链上的属性，遍历字符串会遍历每个字符索引。
     3.2 for...of：迭代对象的键对应的值；适用于**遍历可迭代对象**，如数组、字符串、Set、Map 等，直接遍历元素值，不涉及对象的属性和原型链。(不能直接用于遍历普通对象)，遍历字符串会遍历每个字符值。
