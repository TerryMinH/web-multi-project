...
- [常用 Hooks](https://juejin.cn/post/7118937685653192735)
  ...
  8.  useRef：创建一个可变 ref 对象，这个对象在组件的整个生命周期内不会改变，引用一个不需要渲染的值。
  9.  React.memo:允许组件在 props 没有改变的情况下跳过重新渲染，主要用于优化函数组件的性能，避免不必要的渲染。适用 纯展示组件和 频繁渲染的组件。

  ```js
  // 在 React 内部的简化实现 useCallback 的唯一好处是它可以让你避免在内部编写额外的嵌套函数。
...
- React 原理
  ...
  - React Fiber 架构理解:
    ...
    1. 在 React 16 之前，采用的是栈协调算法，这是一种递归的方式来进行虚拟 DOM 的比较和更新。当组件树比较庞大时，一旦开始协调过程，就会持续占用主线程，直到整个协调完成。在这个过程中，浏览器无法处理其他任务，像用户输入、动画渲染这类操作都会被阻塞，从而导致页面出现卡顿现象。
    2. React Fiber 是 React 16.x 版本之后引入的协调算法架构，其目的是为了解决旧版协调算法在处理大型应用时 React 主线程长时间占用的问题。
  ...
  - React 并发模式（Concurrent Mode）理解
    1. 并发模式是 React 18 版本引入的一种新的渲染策略，通过时间切片，多任务调度，优先级控制等方式实现可中断渲染，提升用户体验。
