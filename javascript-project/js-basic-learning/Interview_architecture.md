 ...
 * @LastEditTime: 2025-03-30 10:57:30
...
- 组件和插件的区别
  1. 组件：主要用于构建用户界面的局部部分，具有局部作用范围和高独立性。一般用于实现特定的 UI 功能和交互，比如按钮、表单、菜单等。组件可以嵌套使用，形成复杂的页面结构。
  2. 插件：主要用于增强应用程序的全局功能，具有全局作用范围和一定的依赖关系。如路由管理、状态管理、日志记录、国际化等，或者对现有组件进行功能扩展。
