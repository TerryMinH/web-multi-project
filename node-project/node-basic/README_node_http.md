 ...
 * @LastEditTime: 2025-04-06 10:09:30
...
- [网络五层模型](https://www.cnblogs.com/terrymin/articles/14007149.html)
...
- HTTP 不同版本区别
  ...
  1. Http1.0：
     ...
     - 默认使用非持久连接
     - 没有专门的头部压缩机制，每次请求和响应都会携带完整的头部信息，包括一些重复的字段，如 User - Agent、Cookie 等
     - 支持的请求方法有限，主要包括 GET、POST 和 HEAD 方法，状态码也相对较少

  2. Http1.1：

     - 默认采用持久连接（Keep - Alive），允许在一个 TCP 连接上发送多个请求和响应，避免了频繁建立和关闭连接的开销。客户端和服务器可以在一次连接中连续发送多个请求，服务器也可以依次返回多个响应，提高了连接的复用性和性能。不过，HTTP 1.1 的持久连接仍然存在队头阻塞问题，即当一个请求被阻塞时，后续的请求也必须等待。
     - 增加了 PUT、DELETE、OPTIONS、TRACE 等多种请求方法，为开发者提供了更多的操作选项。同时，状态码也得到了扩展，能够更准确地表示请求的结果和服务器的状态。

  3. [ Http2.0 与 Http1.1 区别](https://www.cnblogs.com/terrymin/articles/14010138.html)

     - 新的二进制格式：二进制协议，不再是纯文本；将请求和响应拆分为更小的二进制帧进行传输，提高了传输效率和解析速度。
     - 多路复用：HTTP1.1 的连接是串行的,同一时间只能发送一个请求,新的请求必须等待前一个请求完成。HTTP2 允许同时发送多个请求和响应，每个被分成多个帧，并带有唯一的流 ID，这些帧可以乱序发送，接收方根据流 ID 重新组装。多路复用提高了连接的利用率，减少了延迟和头部阻塞的问题。
  ...
  4. HTTP3 有哪些新特性：
     ...
     - 基于 QUIC （Quick UDP Internet Connections）协议：QUIC 协议在 UDP 之上构建，结合了 TCP 的可靠性和 UDP 的低延迟特性，解决了 TCP 连接中的队头阻塞问题。
     - 更快的连接建立：QUIC 协议在建立连接时减少了握手次数，能够更快地建立连接，特别是在移动网络环境下，提高了连接的稳定性和速度。
     - 更好的拥塞控制：QUIC 协议提供了更灵活和高效的拥塞控制机制，能够根据网络状况动态调整数据传输速率，优化网络性能。

- 不同 HTTP 协议关系

  1. HTTP 与 TCP 关系:

     - TCP: 属于传输层协议，主要负责在不同主机的应用程序之间提供可靠的数据传输服务。它通过端口号来标识不同的应用程序，实现了端到端的数据传输。
     - HTTP: 属于应用层协议，主要负责规定客户端和服务器之间数据交互的格式和规则，定义了如何发起请求、如何发送和接收数据以及如何处理响应等。
     - HTTP（超文本传输协议）和 TCP（传输控制协议）:是计算机网络中不同层次的协议，它们之间存在着紧密的关系，共同为网络数据传输和应用层通信提供支持
     - HTTP 依赖 TCP 的连接管理: TCP 负责连接的建立、维护和关闭。在 HTTP 通信中，客户端与服务器之间的每次交互通常都依赖于 TCP 连接。

  2. QUIC（Quick UDP Internet Connections）协议和 UDP（User Datagram Protocol）:

     - QUIC（Quick UDP Internet Connections）协议和 UDP（User Datagram Protocol）均属于传输层协议
     - QUIC 协议：它是 UDP 协议的增强版本，在保留 UDP 低延迟特点的基础上，增加了可靠性、安全性和多路复用等功能，主要应用于 HTTP/3 协议。
     - UDP（User Datagram Protocol）协议：是一种轻量级的传输协议，速度快但缺乏可靠性保障，适用于实时性要求高且能接受数据丢失的场景。
